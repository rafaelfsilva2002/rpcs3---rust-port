...
> **Important post-2026-04-24 update:** the incremental Rust SPU recompiler
> (`rust/rpcs3-spu-recompiler`, R1..R4c) **already exists and is operational**
> — see [`../../docs/PROJECT_STATUS.md`](../../docs/PROJECT_STATUS.md). This
> plan is **no longer a prerequisite for starting the recompiler**. Its
> remaining purpose is to validate interpreter and JIT behavior against
> external RPCS3 traces and real homebrew fixtures, not to gate recompiler
> work that is already done.

**Status:** **P3 + P4 done; pipeline self-test green.** `spu-runner` binário implementado, `spu_homebrew_runner.py --diff` funcional. 8 fixtures sintéticos commitados em `fixtures/spu/` (sentinel, arith, loop, float dot, loadstore, halfword shifts, brsl_ret, orx collapse).
**Owner:** próxima fase = P1 (fixture homebrew real) + P5 (RPCS3 dump capture).
...
3. **SPU interpreter já tem ~126 testes verdes** (~70% ISA, com FTZ denormal flush, halfword/byte ops, channel I/O snapshot, branch family completa) e o Rust SPU recompiler (Cranelift JIT) cobre ~102 opcodes JIT-codegen'd com R4a dispatcher + R4b chained patching + R4c minimal SMC scan. PPU também está representado por um stub de contrato (`rpcs3-ppu-interpreter`), mas a *plataforma ao redor* dele não. Para contagens atuais verificadas, ver [`../../docs/PROJECT_STATUS.md`](../../docs/PROJECT_STATUS.md).
...
- **Recompilador SPU como pré-requisito:** ~~depende dessa fase estar ✅ Forte~~ — **superado**. O Rust SPU recompiler (`rust/rpcs3-spu-recompiler`, Cranelift JIT) já existe (R1..R4c done) e roda 100% via JIT em todos os fixtures sintéticos com `fallback_count = 0`. Ver [`../../docs/PROJECT_STATUS.md`](../../docs/PROJECT_STATUS.md). Esse plano agora valida interpreter+JIT vs RPCS3 C++ em homebrew real, não gateia o recompiler.
