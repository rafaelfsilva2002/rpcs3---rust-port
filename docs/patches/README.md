...
|---|---|---|---|
| [`spu_trace_jsonl_scaffolding.patch`](./spu_trace_jsonl_scaffolding.patch) | C++ trace-writer scaffolding — **R6.7 A.1** (R5.9e.3-fix + new `record_spu_mfc_cmd` / `record_mfc_dma_complete` / `write_dma_chunk_side_file` for MFC GET capture per `docs/SPU_DMA_MFC_R6_7_DESIGN.md` § 4-5) | build-validated against `7028e85fa` through R5.9e.3-fix. R6.7 A.1 not yet rebuild-validated (writer-only design extension; build deferred to A.5 + Phase B). **Current sha256 `cda976d7b7bace826b3e8c38475fab5e077c88201bd0b31768541f06635143a1`** (42,797 bytes). Chronological history: `8525caea…` v1 → `a8baa1a7…b8dbe7b` v2 (R5.8 hardening, seq race fix) → `2baebca5…91149` (R5.9c, target_spu on 7 SPU-side recorders) → `d4873c358d…509a09ac` (R5.9e.3, +`record_spu_image` + `mbedtls_sha256` + `.spuimg` side-file write) → `d65aec91…ae1aba1c` (R5.9e.3-fix, write_image_side_file returns bool + post-write file_size verification + record_spu_image bails before JSONL on failure; trace v4 capture validated end-to-end with `.spuimg` SHA-matching the JSONL `image_sha256`) → current (R6.7 A.1, +`SpuMfcCmd` / `MfcDmaComplete` event-kind enum entries, +`record_spu_mfc_cmd(target_spu, pc, cmd, tag, size, lsa, eah, eal, ea_bytes, ea_bytes_size)` with mandatory-side-file SHA-256 + `<trace>.dma/<sha>.dmachunk` content-addressed write before JSONL emit, +`record_mfc_dma_complete(target_spu, tag, transferred_bytes)`, +`write_dma_chunk_side_file` private helper mirroring `write_image_side_file` for `.dmachunk` files). | [details below](#spu_trace_jsonl_scaffolding) |
| [`spu_trace_jsonl_runtime_hooks.patch`](./spu_trace_jsonl_runtime_hooks.patch) | Runtime hooks at SPU/PPU hot-paths — **R6.7 A.1** (R5.9e.3 + new MFC channel hooks: ch16-20 + ch22-23 wrch, ch24 rdch fast-path, ch21 wrch + EA snapshot via `vm::_ptr` BEFORE `process_mfc_cmd()` + `record_spu_mfc_cmd` + `record_mfc_dma_complete`) | build + smoke validated through R5.9d real-trace v3. R6.7 A.1 not yet rebuild-validated (build deferred to A.5 + Phase B). **Current sha256 `95bdcaae4850f3b2a94b5aea59761263589efabeac71bd3cb8464ad59c3a6721`** (18,359 bytes). Chronological history: `1b69f107…b28694` (R5.8 A.3 final) → `3ee7a861…2bed39` (R5.9c, `lv2_id` threaded through 11 SPU-side call sites + 2 lambda-scope snapshots) → `8f253d7d…66663a` (R5.9e.3, +new cpu_task hunk emitting `spu_image` after `pc &= 0x3fffc;` so entry_pc is the clean instruction address; +6 hunk-header offset adjustments) → current (R6.7 A.1, +three new SPUThread.cpp hunks: MFC_RdTagStat fast-path rdch hook reusing `trace_target_spu`/`trace_pc` from the R5.8 lambda-scope snapshots; MFC_WrTagMask wrch hook; MFC_WrTagUpdate / MFC_LSA / MFC_EAH / MFC_EAL / MFC_Size / MFC_TagID wrch hooks merged into one hunk; MFC_Cmd wrch + EA snapshot via `vm::_ptr<u8>(eal)` BEFORE `process_mfc_cmd()` + post-dispatch `mfc_dma_complete` for plain GET only — non-GET cmd codes emit only the bare `spu_wrch` and the parser will reject them with `UnsupportedMfcCmd` per R6.7 design § 4.3). | [details below](#spu_trace_jsonl_runtime_hooks) |
