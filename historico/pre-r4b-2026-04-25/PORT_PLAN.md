...
> Frozen snapshot at: [`PLAN_FREEZE_2026-04-24.md`](PLAN_FREEZE_2026-04-24.md)
> Current numbers: **230 crates / 5165 tests / 229 iters / 0 regressions**. See [`CURRENT_STATE.md`](CURRENT_STATE.md).
> Plan is **substantially complete as scope artifact** — runtime parity is NOT complete (ADR-011).
> Initial estimates below are kept for historical reference; sections marked ⏪ have superseded values.
...
> ⏪ **Initial estimate, now superseded by current state:** **0 Zig crates committed** in the wave. None of the ports hit the "measurable benefit" bar (see ADR-009 in [`DECISIONS.md`](DECISIONS.md)). Door remains open for hot-path Zig in future waves.

**Total de crates Rust:** 25-35 (a maioria do projeto).
> ⏪ **Initial estimate, now superseded by current state:** **230 crates** (as of 2026-04-24). The original 25-35 estimate was based on a "high-level module" granularity; the actual port chose finer granularity (one crate per cpp file in most cases), which gives both better testability and a clearer audit trail. See [`CURRENT_STATE.md`](CURRENT_STATE.md).

**C++ remanescente:** tudo não-portado ainda. Meta é levar a zero, mas JIT LLVM e RSX Vulkan são os últimos pedaços (Ondas 8-9).
> ⏪ **Initial estimate, now superseded by current state:** see [`DEFERRED.md`](DEFERRED.md) items 6-9. JIT (SPU/PPU Recompilers, PPU Translator) and RSX runtime (RSXThread, VKGSRender) remain explicitly out of scope as multi-week dedicated waves. Behavior-freeze contracts in `rpcs3-ppu-interpreter` / `rpcs3-spu-interpreter` / `rpcs3-ppu-thread` / `rpcs3-spu-thread` cover what this wave needs.
...
> ⏪ **Initial estimate, partially superseded by current state (2026-04-24):**
> - Phases 0-2 + part of 3 + LV2 first wave + HLE MVP **port surface** delivered in **a single multi-session sprint** (229 autonomous iterations, 230 crates, 5165 tests). The original 12-18 month estimate was for a single full-time human; the actual delivery used assisted autonomous execution.
> - Phases 5-8 (sentinel game running through HLE Rust + JIT + RSX + GUI) **remain unfulfilled**. The "3+ anos paridade completa" estimate is still relevant for those waves — see [`DEFERRED.md`](DEFERRED.md) items 6-8.
> - "Plan substantially complete" ≠ "runtime parity complete". See ADR-011 in [`DECISIONS.md`](DECISIONS.md).
