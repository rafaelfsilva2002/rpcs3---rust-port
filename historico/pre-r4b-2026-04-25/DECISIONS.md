...
Decisions here are the load-bearing ones — anything that, if reversed, would
require redoing significant work.
...
- **Consequence:** JIT/RSX/Qt remain explicitly out of scope (see [`DEFERRED.md`](DEFERRED.md)) until someone signs up for a multi-week dedicated wave.
...
- **Consequence:** The port is currently 100% Rust + the original C++ source. Door remains open for Zig in future hot-path work (e.g. SPU recompiler, RSX command stream).
...
- **Consequence:** README, CURRENT_STATE, and CHECKLIST all repeat this clarification verbatim. See [`CURRENT_STATE.md`](CURRENT_STATE.md).
...
- **Rationale:** Without a baseline, "we're making progress" is unfalsifiable. The frozen numbers + the snapshot files (`PLAN_FREEZE_2026-04-24.md`, `CHECKLIST_FREEZE_2026-04-24.md`, `CURRENT_STATE_2026-04-24.md`) make the baseline reproducible.
- **Consequence:** Future work either updates these counters in [`CURRENT_STATE.md`](CURRENT_STATE.md) or it's not landed. The freeze snapshots are kept as historical reference and never edited.
