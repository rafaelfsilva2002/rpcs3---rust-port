...
## Onda de hardening + iter-8 SPU (2026-04-24, pós-freeze)

### Hardening (4 otimizações byte-exact, zero-regressão)
- `split_lanes` — 16 byte-copies → 4 bit-shifts (`const fn`).
- `join_lanes` — 4 `copy_from_slice` → 3 OR-shifts (`const fn`).
- `broadcast_u32` — `join_lanes([v;4])` → composição direta.
- `read_qword_be` — `[0u8;16] + copy_from_slice` → `try_into`.

Decisões descartadas com justificativa:
- ❌ Restruturar cascata `bits(inst, 0, 11)`: compilador já faz CSE, ganho real ~0, risco de bug em decoder denso.
- ❌ Trocar `iter().find()` em ENTRY_POINTS por hash: tabelas <16 entradas — linear scan já é ótimo.
- ❌ `to_string()` / `Vec::new` mass cleanup: paths de erro/init, não estão em hot loop.

### Iter-8: 17 novas instruções SPU (+21 testes, 68 → 89)

**Single-precision compares** (cpp:1131..1265 fast path com denormal flush):
- FCGT (0x2C2), FCMGT (0x2CA), FCEQ (0x3C2), FCMEQ (0x3CA)

**Reciprocal estimates** (LUT-based no cpp; nosso path inicial é naïve):
- FREST (0x1B8), FRSQEST (0x1B9)
- TODO: `spu-frest-lut` para byte-exact com LUTs do cpp:690..735

**FM melhorado** (cpp:1192) — denormal-flush adicionado ao `0x2C6`.

**Form select mask:**
- FSM (0x1B4) — bits 0..3 do preferred slot expandem para 4 word-lanes.

**Indexed load/store** (cpp:738..742):
- LQX (0x1C4), STQX (0x144) — endereço = (ra[0] + rb[0]) & 0x3FFF0

**Indirect branches** (cpp opcodes 0x128..0x12B, 0x1A8..0x1AC):
- BI, BISL (com link), IRET (= BI sem IRQ), HBR (NOP no interpretador)
- BIZ, BINZ, BIHZ, BIHNZ (com condição em rt)

**Tests:** 21 novos testes cobrindo cada família. Workspace: 5165 → **5186, zero regressão.**

### Scaffold homebrew validation
- `harness/spu_homebrew_runner.py` — placeholder Python com docstring.
- `docs/HOMEBREW_PLAN.md` — roadmap detalhado (P1..P5, estimativa 6-16h).
- Bloqueado por: fixture SPU ELF + crate `spu-runner` + crate `rpcs3-spu-elf-loader`.

---

## Phase B execution — P3+P4 done (2026-04-24)

### Decisão: P2 reuse, sem crate novo
- `rpcs3-loader-elf-self::parse_elf` (via `goblin`) já cobre ELF SPU 32-bit BE.
- `ElfInfo::is_spu()` e `pt_load_iter()` expostos.
- ❌ Cancelei criação de `rpcs3-spu-elf-loader` — duplicação evitada.

### P3: crate binário `spu-runner`
- `rust/spu-runner/` (workspace member novo).
- ~170 LOC main.rs + 5 integration tests (smoke.rs).
- Lê ELF → popula LS → roda interpreter → dumps {gpr.csv, pc.txt, ls.bin, summary.txt}.
- Exit codes documentados (0 Stop / 1 max-steps / 2 interpreter err / 3 load err / 64 CLI).
- Tests cobrem: happy-path com ELF sintético; non-SPU ELF; missing file; CLI usage; max-steps.

### P4: harness Python diff completo
- `harness/spu_homebrew_runner.py` — modos run/diff/comparativo.
- `diff_dumps()` reporta diferenças por GPR linha-a-linha + PC + chunks de LS de 16 bytes (5 primeiros).
- `harness/test_spu_homebrew_runner.py` — self-test que monta ELF in-Python, roda 2× via subprocess, diffa, espera IDENTICAL.

### Fixture commitada
- `fixtures/spu/synthetic_il_stop.elf` (92 bytes) — `il r3, 0x1234; stop 0`. Sentinel reproducível para CI/regressão.

### Estado workspace
- **5186 lib tests + 5 integration tests do spu-runner = 5191 testes verdes.**
- Zero regressão.
- Phase B agora bloqueia apenas em P1 (homebrew real) e P5 (dump capture do RPCS3 C++).

---

## Iter-9 SPU + Fixture coverage expansion (2026-04-25)

### Novos opcodes SPU (+17, 89 → 107 testes)
- **Vector word shifts** (cpp:287..339, 396..453): SHL (0x05B), ROT (0x058), ROTM (0x059), ROTMA (0x05A) + immediates ROTI (0x078), ROTMI (0x079), ROTMAI (0x07A).
- **Bitwise complementaries** (cpp:487, 1026, 1124, 1230): NAND (0x0C9), EQV (0x249), ANDC (0x2C1), ORC (0x2C9).
- **Extended compares** (cpp:1019, 1485, 1516 + variantes): CEQH (0x3C8), CEQB (0x3D0), CGTH (0x248), CGTB (0x250), CLGTH (0x2C8), CLGTB (0x2D0).
- **Barriers + alias stops**: SYNC (0x002), DSYNC (0x003) — NOPs no interpretador. STOPD (0x140) — alias para `stop 0`.

### Novos helpers internos
- `shl_word` / `shr_word` / `sar_word` — count masked to 6 bits, ≥32 → 0 (ou sign-fill em SAR).
- `shr_const` / `sar_const` — variantes immediate (count já canonical).

### Bug encontrado e corrigido
- Encoder Python `stop()` em harness/ tinha shift errado (`<< 7` em vez de `& 0x3FFF`). Detectado quando os stop codes dos novos fixtures não bateram com o esperado. Corrigido também no `smoke.rs` Rust integration test (estava igualmente errado mas o código=0 não revelava o bug).

### Fixtures sintéticos reproduzíveis (4 commitados)
- `harness/build_synthetic_fixtures.py` — gerador determinístico.
- `synthetic_il_stop.elf` (92 B) — sentinel.
- `synthetic_arith.elf` (120 B) — exercita ALU integer + bitwise + shift.
- `synthetic_loop.elf` (116 B) — **soma 1+2+...+10 = 55** via loop com brnz back-edge. Comprova que branches funcionam em ciclo real.
- `synthetic_float_dot.elf` (112 B) — chain `fa→fm→fa` produz float 8.0 (0x41000000).

### Novo integration test set (`spu-runner/tests/fixtures.rs`)
- 4 testes que executam cada fixture e verificam GPRs + stop code esperados.
- Detecta divergências em opcode semantics, encoder format ou loader behavior.

### Estado workspace
- **5204 lib + 9 integration = 5213 testes verdes** (era 5191 antes desta sub-onda).
- Zero regressão.
- Cobertura SPU subiu de ~35% (post iter-8) para ~50% (post iter-9).

---

## Iter-10 SPU + Recompiler pre-flight infra (2026-04-25)

### Inventário de lacunas (gates para o futuro recompiler)
1. **Differential harness não-trait-based** — gap fechado nesta onda.
2. **FPSCR não modelado** — TODO documentado em SPU_RECOMPILER_PLAN.md (gate para R3).
3. **Channel snapshot incompleto** — `ChannelCounts` default-zero hoje; TODO documentado.
4. **Cobertura SPU ainda < 60%** — fechando para 60%+ nesta onda.

### Iter-10 SPU (+11 testes, 107 → 118)
- **Halfword arith:** AH (0x0C8), SFH (0x048).
- **Carry/borrow generate:** CG (0x0C2), BG (0x042).
- **OR-across:** ORX (0x1F0).
- **Branch+link relative:** BRSL (9-bit primary 0x066).
- **Branch hints (NOPs):** HBRA (7-bit 0x08), HBRR (7-bit 0x09).
- **Halfword imm compares:** CEQHI (8-bit 0x7D), CGTHI (0x4D), CLGTHI (0x5D).

### Crate novo: `rpcs3-spu-differential` (12 tests)
- `SpuExecutor` trait — backend-agnostic execution interface.
- `SpuProgram` (entry + segments + max_steps), `validate()` checks bounds + alignment.
- `SpuExecutionResult` { steps_executed, stop_reason, final_state }.
- `SpuStateSnapshot` { pc, gpr (Box<[u128;128]>), ls (Box<[u8;256K]>), channel_counts }.
- `ExecutionStopReason` { Stop, ChannelStall, MaxStepsExceeded, Error }.
- `InterpreterExecutor` — reference oracle.
- `diff_snapshots()` — GPR + PC + LS + channel diff.
- `run_and_diff()` — pair two backends, diff results.
- Recompiler futuro implementa o mesmo trait → validação automática.

### Fixtures expandidas (4 → 8)
- `synthetic_loadstore.elf` (104 B) — round-trip stqd → lqd.
- `synthetic_shifts.elf` (112 B) — exercita shli/rotmi/rotmai/roti.
- `synthetic_brsl_ret.elf` (108 B) — function call via brsl + bi (link register).
- `synthetic_orx_collapse.elf` (104 B) — ah + orx (per-halfword + collapse).

### `spu-runner` refactored
- Agora chama backend via `SpuExecutor` trait.
- Flag `--backend interpreter` (default; `--backend recompiler` quando R2 ficar pronto).
- Removida dependência direta do interpreter; agora vem via `rpcs3-spu-differential`.
- Helpers `build_program_from_elf` + `extract_segment` com validação byte-bound.

### Documento `docs/SPU_RECOMPILER_PLAN.md`
- Inventário do C++ SPUCommonRecompiler.cpp (9792 linhas, mapeado por seção).
- Decisões D1-D5 (codegen backend, trait, cache, analysis, fallback).
- **Recomendação backend: Cranelift** (vs LLVM/inkwell, vs hand-rolled). Justificativa explícita.
- Fases R0-R5 com estimativas e gates.
- Riscos enumerados (FPSCR, channel snapshot, branch chaining, self-modifying code, LLVM-on-Windows).
- "Como começar Phase R1" — passos concretos.

### Estado workspace
- **5227 lib + 13 integration = 5240 testes verdes** (era 5213).
- Zero regressão.
- 232 crates (era 231).
- Cobertura SPU: ~60% das instruções (era ~50% post iter-9).
- **Pronto para iniciar Phase R1 do SPU recompiler.**

---

## Phase R1 + R2: Decoder + Recompiler scaffold (2026-04-25, loop autônomo)

### R1: `rpcs3-spu-decoder` (28 tests)
- 20 lib tests + 8 fixture-driven integration tests.
- `decode_inst(raw, pc)` — single-instruction decode com `SpuInstKind` enum por família semântica.
- `decode_function(ls, entry, max_blocks)` — **two-pass leader analysis**:
  - Pass 1: DFS para coletar todos os PCs alvo de branch + entry.
  - Pass 2: cortar blocos em cada leader (blocos não se sobrepõem).
- `BlockTerminator` enum: Stop / UncondDirect / UncondIndirect / CondDirect / CondIndirect / UnknownOpcode / FellThroughLimit.
- Integration tests validam que cada um dos 8 fixtures decodifica em estrutura coerente (todos os branches diretos pousam dentro de leaders conhecidos).

### R2: `rpcs3-spu-recompiler` (7 lib + 4 spu-runner integration)
- `RecompilerExecutor` implementa `SpuExecutor` trait do `rpcs3-spu-differential`.
- Function cache via `HashMap<(entry_pc, ls_hash_256_bytes), SpuFunction>`.
- Hoje **delega ao interpreter** após decode + cache → byte-identical por construção.
- `spu-runner --backend recompiler` wireado.
- 4 differential integration tests confirmam interpreter ≡ recompiler em fixtures arith/loop/brsl_ret/il_stop.
- Pronto para receber backend Cranelift (R2.5) plug-in interno em `execute()`.

### Bug crítico fixado durante R1
- 3 opcodes (A=0x180, AND=0x181, SF=0x080) eram **2× canonical SPU ISA** (0xC0, 0xC1, 0x40).
- Bug interno consistente — todos os 5240 testes pré-fix passavam — mas byte-incompatible com binutils SPU asm e RPCS3 C++.
- Detectado quando o decoder (escrito a partir do C++) não casava com bytes do encoder (escrito a partir do estado pre-existente).
- Fix: encoder + dispatcher + Python fixture builder atualizados; fixtures regeneradas; **zero regressão**.
- Resultado: port agora byte-compatible com encoding canonical → futuro homebrew real "just works".

### Estado workspace pós-R1+R2
- **5254 lib + 17 spu-runner + 28 decoder = 5299 testes verdes**.
- Zero regressão.
- **234 crates** (era 232 — added decoder + recompiler scaffold).
- **Phase R0+R1+R2 completos.** Próximo gate: **R2.5 = Cranelift codegen** (precisa decisão arquitetural do user para adicionar dep `cranelift`, ~5MB pure-Rust). Caminho concreto documentado em SPU_RECOMPILER_PLAN.md.

---

## Iter-11 + R3 prep (2026-04-25, loop autônomo continuado)

User disse "pode continuar" — interpretação conservadora: NÃO atravessei a regra "Cranelift requer aprovação", ataquei R3 blockers e cobertura.

### Channel snapshot resolved (R3 blocker)
- `snapshot_from_thread` agora popula `ChannelCounts` direto dos campos públicos de `SpuChannels`.
- 2 testes novos em `rpcs3-spu-differential`:
  - `channel_counts_reflect_outbox_write`: programa il+wrch+stop deixa `out_mbox_depth=1`.
  - `channel_counts_diff_when_one_run_writes_outbox`: diff harness reporta divergência.
- Total: 12 → 14 lib tests.

### Iter-11: 8 halfword shifts (SPU 118 → 126)
- **RR-form (4):** ROTH (0x05C), ROTHM (0x05D), ROTMAH (0x05E), SHLH (0x05F).
- **RI7-form (4):** ROTHI (0x07C), ROTHMI (0x07D), ROTMAHI (0x07E), SHLHI (0x07F).
- Helpers `halfword_op` / `halfword_const_op` reutilizáveis.
- 8 unit tests cobrindo todos eles.

### Novo fixture sintético: `synthetic_halfword_shifts.elf`
- 5 instruções: ilh + shlhi + rothmi + rothi + stop.
- Mostra 0x00FF broadcast virando 0x0FF0 / 0x000F / 0xFF00 (shift left 4 / shift right 4 / rotate 8).
- 3 spu-runner integration tests: básico + recompiler-vs-interpreter diff (interpreter ≡ recompiler scaffold).

### Estado workspace
- **5264 lib + 19 spu-runner + 28 decoder = 5311 testes verdes**.
- Zero regressão (10ª onda consecutiva).
- 234 crates.
- SPU coverage agora ~70% (era ~60% post-iter-10).
- **R3 blockers restantes:** apenas FPSCR (não bloqueador imediato — nenhum fixture atual usa FSCRRD/FSCRWR, então diff harness não silenciosa nada).

---

## Phase R2.5: Cranelift JIT FIRST LIGHT (2026-04-25, user disse "A")

User aprovou Cranelift como backend. Implementação real, byte-exata por construção.

### Deps adicionadas
- `cranelift = "0.118"` + `cranelift-jit` + `cranelift-module` + `cranelift-native` + `target-lexicon = "0.13"`.
- Pure-Rust, ~5MB, zero deps externas (nenhum LLVM/MSVC tooling extra).
- Primeiro build cold: ~47s. Builds incrementais: <2s.

### `rpcs3-spu-recompiler/src/jit.rs` (~400 LOC)
- `JitState` repr(C): 128 GPRs × 4 lanes u32 + pc + stop_code (memory layout fixo via `offset_of!`).
- `JitBackend`: owns `JITModule`, `compile_at(ls, entry_pc)` retorna `CompiledFunction`.
- `CompiledFunction::call(state)` — `extern "C" fn(*mut JitState) -> u32`.
- Outcomes: `JIT_OUTCOME_STOP` / `JIT_OUTCOME_BAILOUT` / `JIT_OUTCOME_STALL`.
- **Pre-flight check** (`supported_check`) varre todos os opcodes do bloco antes de tocar Cranelift; falha rápido com `JitError::Unsupported`.

### Subset codegen suportado (FIRST LIGHT)
- `stop code` — write pc+stop_code, return STOP.
- `nop` / `lnop` — no-op.
- `il rt, imm16` — sign-extended broadcast 4 lanes.
- `ila rt, imm18` — 18-bit unsigned broadcast.
- `a/sf/and/or/xor/nor rt, ra, rb` — per-lane word op.

### `RecompilerExecutor` agora JIT-first
- `JitCache` interno: map de programs compilados + `JitStats` (jit_runs, fallback_runs, compiled_functions).
- `try_jit_run`: pre-flight + compile + execute. Bail-outs ou erros de compile → fallback transparente ao interpreter.
- "Tried & failed" cached: programas com opcode unsupported não recompilam toda vez.
- Backend name agora é `recompiler-jit` (era `recompiler-scaffold`).

### Bug encontrado e corrigido durante desenvolvimento
- `emit_stop` advance PC para pc+4, mas o interpreter NÃO advance no stop. Diff harness pegou imediatamente. Fix: store pc do próprio stop instruction.

### Tests verde
- 14 lib tests em `rpcs3-spu-recompiler` (era 7).
- 4 spu-runner differential tests confirmam JIT ≡ interpreter byte-a-byte em fixtures arith/loop/brsl_ret/il_stop (mesmo que fixtures não-trivial caiam pro fallback, o resultado bate).

### Métricas observáveis via JitStats
- `jit_runs`: quantas execuções rodaram pelo JIT.
- `fallback_runs`: quantas caíram para o interpreter.
- `compiled_functions`: quantas funções únicas foram JIT-compiladas (cache size).
- 1 test (`jit_stats_track_runs_and_compilations`) valida o tracking.

### Estado workspace pós-R2.5 first light
- **5271 lib + 19 spu-runner + 28 decoder = 5318 testes verdes**.
- Zero regressão (11ª onda consecutiva).
- 234 crates (mesma — JIT cresceu dentro do crate existente, não criou novo).
- **JIT real funciona.** Próxima sub-onda: expandir subset (branches, load/store, restantes ALU) — cada novo opcode segue padrão `supported_check + emit_inst + differential test`.

---

## Phase R2.5 MULTI-BLOCK + branches (2026-04-25, loop continuação)

User disse "pode continuar em loop ai" — expansão JIT incremental.

### Refatoração: multi-block compile
- `JitBackend::compile` agora cria uma Cranelift `Block` por SPU basic block.
- Usa `cranelift::frontend::Variable` para carregar `*mut JitState` entre blocks (declared once, def_var em entry, use_var nos demais).
- `seal_all_blocks()` no final permite loops (back-edges) sem ordem manual de selagem.
- `emit_block` gera instructions + terminator separadamente: instruções normais via `emit_inst`, terminator via match em `BlockTerminator`.

### Pre-flight expandido (rejeita funções incompiláveis)
- `check_block_supported`: rejeita blocks com terminator `UncondIndirect` / `CondIndirect` / `UnknownOpcode` / `FellThroughLimit`.
- Resultado: brsl_ret (que tem `bi r5`) cai 100% pro interpreter — correto, sem perda.
- arith / loop / il_stop são totalmente JIT'áveis.

### Novos opcodes JIT (de 11 → ~25)
- **Branches diretos:**
  - `br i16` (0x064) → `builder.ins().jump(target_block)`
  - `bra i16` (0x060) → idem (absolute)
  - `brnz rt, i16` (0x042) → load lane 0, `icmp NotEqual 0`, `brif`
  - `brz rt, i16` (0x040) → load lane 0, `icmp Equal 0`, `brif`
  - `brsl rt, i16` (0x066) → write link (broadcast pc+4) + jump
- **RI10 ALU/cmp:** `ai`, `andi`, `ori`, `xori`, `ceqi`, `cgti`
- **LoadImm completo:** `ila`, `ilh`, `ilhu`, `iohl`
- Helpers reutilizáveis: `emit_word_op` (ALU) + `emit_word_imm` (RI10).

### Demo: synthetic_loop.elf via JIT puro
- Programa: `ila r3,0; ila r4,1; loop: a r3,r3,r4; ai r4,r4,1; ceqi r5,r4,11; brnz r5,exit; br loop; exit: stop`
- 4 SPU basic blocks (0x100, 0x108 loop body, 0x118 back-edge, 0x11C exit).
- JIT compila tudo, executa loop completo (10 iterações), produz r3 = 55 = 0x37 broadcast.
- **JitStats**: `compiled_functions = 1`, `jit_runs = 1`, `fallback_runs = 0`. Loop completo via JIT, **ZERO fallback ao interpreter**.
- 2 testes lib confirmam: `jit_runs_full_loop_program_no_fallback` + `jit_runs_full_loop_program_byte_identical_to_interpreter`.

### Bug encontrado durante codegen
- `Variable::new(0)` exige `use cranelift::prelude::EntityRef;` para resolver o trait method. Compilador deu hint claro; fix de 1 linha.

### Estado workspace pós-MULTI-BLOCK
- **5277 lib + 19 spu-runner + 28 decoder = 5324 testes verdes**.
- Zero regressão (12ª onda consecutiva).
- 234 crates (sem novos).
- JIT cobertura: ~25 opcodes (2× a anterior).
- **synthetic_loop.elf** rodando 100% via JIT — primeira prova de loops byte-exatos no path JIT.
- Próximas frentes (incremental, sem novo gate): load/store qword, compares restantes, halfword ops, float family, indirect branches via dispatcher.

---

## Phase R2.5 EXPANSION — RI7 shifts + RR compares + RR shifts + bitwise extras (2026-04-25)

User: "pode continuar em loop ai" — sub-onda de expansão JIT incremental.

### Novos opcodes JIT (~15 novos, total ~40)
- **RI7 word shifts:** shli (0x07B), roti (0x078), rotmi (0x079), rotmai (0x07A).
- **RR word compares:** ceq (0x3C0), cgt (0x240), clgt (0x2C0). Mask 0xFFFFFFFF / 0 via select.
- **RR word shifts dinâmicos:** shl (0x05B), shr (0x059, ROTM mnemonic), sar (0x05A, ROTMA), rot (0x058). Implementação via `select(count >= 32, saturate, shift)` para emular SPU "0 / sign-fill" em count >= 32 (vs x86 que faz `count mod 32`).
- **Bitwise extras:** nand (0x0C9), eqv (0x249), andc (0x2C1), orc (0x2C9).
- **Carry/borrow:** cg (0x0C2) via uextend para u64, somar, shr 32; bg (0x042) via select(LessOrEqual).

### Helpers reutilizáveis
- `emit_cmp_to_mask(builder, cc, av, bv) -> Value` — pattern compartilhado para todos os compares.
- `emit_dyn_shift_word(builder, av, bv, kind)` — dynamic shift com SPU saturação semantic.
- `emit_word_const_shift(builder, state, rt, ra, n, kind)` — shift constante (n já canonicalizado pelo caller).

### Demo: synthetic_arith.elf via JIT puro
- Programa: `il r3,5; il r4,3; a r5,r3,r4; sf r6,r4,r3; shli r7,r3,2; xor r8,r3,r4; or r9,r3,r4; and r10,r3,r4; stop`
- Antes: shli não suportado → fallback ao interpreter
- Agora: **100% via JIT, zero fallback**. Test `jit_runs_full_arith_program_no_fallback` confirma `compiled_functions=1, jit_runs=1, fallback_runs=0`.
- Differential test `jit_runs_full_arith_program_byte_identical_to_interpreter` valida byte-exatness vs interpreter.

### Estado workspace pós-EXPANSION
- **5284 lib + 19 spu-runner + 28 decoder = 5331 testes verdes**.
- Zero regressão (13ª onda consecutiva).
- 234 crates.
- JIT cobertura: ~40 opcodes.
- **2 fixtures sintéticos rodam 100% via JIT** (loop + arith); demais ainda caem pro interpreter por opcodes específicos (load/store, halfword, float, etc.).
- Próximas frentes incrementais: load/store qword (necessita acesso ao LS via host pointer); halfword/byte ops; float family.

---

## Phase R2.5 LOAD/STORE — JIT acessa LS via ls_ptr (2026-04-25)

### Mudança no JitState
- Adicionado `ls_ptr: *mut u8` ao struct `repr(C)`.
- `JitState::new()` deixa null; tests configuram manualmente quando precisam de LS access.
- `RecompilerExecutor::try_jit_run` agora aloca `Box<[u8; 256K]>` para o LS, popula com program.segments, e configura `state.ls_ptr` antes de chamar `compiled.call`. Após o retorno, null o pointer (defesa contra dangling).

### Codegen lqd/stqd/lqx/stqx
- `emit_qword_dform`: addr = (ra preferred + imm10*16) & 0x3FFF0
- `emit_qword_indexed`: addr = (ra preferred + rb preferred) & 0x3FFF0
- `emit_qword_xfer`: 4× load/store i32 entre LS e GPR lanes, com `bswap` para conversão BE↔LE.
- Helper `emit_ls_target`: load `ls_ptr` from state + uextend(addr) + iadd → host pointer.

### Demo: synthetic_loadstore.elf 100% via JIT
- Programa: `il r3, 0x5A5A; ila r4, 0x40; stqd r3, 1(r4); lqd r5, 1(r4); stop 0xAB`
- JIT escreve r3 broadcast pattern para LS @ 0x50, lê de volta para r5.
- Test confirma `compiled_functions=1, jit_runs=1, fallback_runs=0`.
- Test diff vs interpreter passa byte-exato (incluindo o LS dump).

### Estado workspace pós-LOAD/STORE
- **5287 lib + 19 spu-runner + 28 decoder = 5334 testes verdes**.
- Zero regressão (14ª onda consecutiva).
- 234 crates.
- JIT cobertura: ~44 opcodes.
- **3 fixtures sintéticos rodam 100% via JIT** (loop, arith, loadstore — mais o trivial il_stop = 4).
- Próximas frentes: halfword/byte ops, float family, indirect branches via dispatcher.

---

## Phase R2.5 HALFWORD + ORX (2026-04-25)

User: "pode continuar em loop ai" — sub-onda focada em desbloquear synthetic_orx_collapse.

### Novos opcodes JIT (~3, total ~47)
- **AH (0x0C8)** — per-halfword add (8 lanes u16). Implementação via técnica split-mask-add-repack: split cada lane u32 em hi (>> 16) e lo (& 0xFFFF), soma cada par independentemente, mascara overflow para 16 bits, re-pack.
- **SFH (0x048)** — per-halfword sub-from. Mesma técnica, com `isub(bv, av)` (`rt = rb - ra` no SPU).
- **ORX (0x1F0)** — Unary OR-across word lanes para preferred slot. Carrega 4 lanes, OR de tudo, store no lane 0 do rt; lanes 1-3 = 0.

### Helper reutilizável
- `emit_halfword_arith` com enum `HalfArith { Add, SubFrom }` — futura expansão (mph/mphh/etc) reusará o pattern.

### Bug encontrado durante test design
- Test inicial assumia que `il r3, 0x1234` põe 0x1234 em ambas halfwords do lane. Errado: `il` faz sign-extend da imm16 para 32 bits, broadcast para 4 lanes; cada lane fica `0x00001234` (high half = 0, low half = 0x1234). Test corrigido para refletir semântica real.
- Adicionei outro test (`jit_ah_handles_halfword_wrap`) que pre-popula GPRs com pattern 0xFFFF_0001 + 0x0001_FFFF para confirmar wrap correto a nível de halfword.

### Demo: synthetic_orx_collapse.elf 100% via JIT
- Programa: `il r3, 0x1234; il r4, 0x5678; ah r5, r3, r4; orx r6, r5; stop 0xCC`
- Antes: AH e ORX não suportados → fallback ao interpreter
- Agora: **100% via JIT, zero fallback**. r6 lane 0 = 0x000068AC.
- 2 testes lib (`no_fallback` + `byte_identical`).

### Estado workspace pós-HALFWORD+ORX
- **5292 lib + 19 spu-runner + 28 decoder = 5339 testes verdes**.
- Zero regressão (15ª onda consecutiva).
- 234 crates.
- JIT cobertura: ~47 opcodes.
- **4 fixtures sintéticos rodam 100% via JIT** (loop, arith, loadstore, orx_collapse — mais il_stop trivial = 5/9 fixtures totais).
- Fallback restante: synthetic_halfword_shifts (precisa halfword shifts), synthetic_brsl_ret (precisa indirect branches), synthetic_float_dot (precisa float family).

---

## Phase R2.5 HALFWORD-SHIFTS (2026-04-25)

User: continuação loop. Sub-onda focada em desbloquear synthetic_halfword_shifts.

### Novos opcodes JIT (~4, total ~51)
- **SHLHI (0x07F)** — shl per halfword imm.
- **ROTHI (0x07C)** — rotate-left per halfword imm.
- **ROTHMI (0x07D)** — logical shr per halfword imm.
- **ROTMAHI (0x07E)** — arith shr per halfword imm (com sextend para preservar sign).

### Helper `emit_halfword_const_shift`
- Split-mask-shift-repack pattern (similar ao emit_halfword_arith).
- Para arith shr: sextend i16→i32 antes de sshr, depois mask i16.
- Para rotate: simulação via `(av << n) | (av >> (16-n))` mascarado.
- Reusa `ConstShift` enum existente do word-shift path.

### Bug encontrado durante testing
- Test inicial caiu pra interpreter (fallback_runs=1 em vez de 0).
- Root cause: **decoder não classificava 0x07C..0x07F como AluImm7** — caía em `Unclassified`. Pre-flight do JIT rejeitava.
- Fix: estender o `matches!` em `decode_inst` (rpcs3-spu-decoder/src/lib.rs) para incluir os 4 halfword shift primaries.
- Decoder integration tests permaneceram verde (28/28) — bugfix não introduziu regressão.

### Bug Cranelift dual mutable borrow
- `builder.ins().sextend(I32, builder.ins().ireduce(...))` — chamada aninhada produz dois mutable borrows simultâneos do builder. Compilador detectou.
- Fix: extrair em variáveis intermediárias.

### Demo: synthetic_halfword_shifts.elf 100% via JIT
- Programa: `ilh r3, 0x00FF; shlhi r4, r3, 4; rothmi r5, r3, -4; rothi r6, r3, 8; stop 0xDD`
- r4 = 0x0FF0_0FF0 broadcast (shl 4 per halfword)
- r5 = 0x000F_000F broadcast (shr 4 per halfword)
- r6 = 0xFF00_FF00 broadcast (rotate 8 per halfword)
- Test confirma `fallback_runs=0`, byte-exato vs interpreter.

### Estado workspace pós-HALFWORD-SHIFTS
- **5294 lib + 19 spu-runner + 28 decoder = 5341 testes verdes**.
- Zero regressão (16ª onda consecutiva).
- 234 crates.
- JIT cobertura: ~51 opcodes.
- **5 fixtures sintéticos rodam 100% via JIT** (loop, arith, loadstore, orx_collapse, halfword_shifts — mais il_stop trivial = 6/9 fixtures totais).
- Fallback restante: synthetic_brsl_ret (indirect branches), synthetic_float_dot (float family).

---

## Phase R2.5 FLOAT-FAMILY (2026-04-25, continuação loop)

User: continuação loop. Sub-onda focada em desbloquear synthetic_float_dot.

### Novos opcodes JIT (~3, total ~54)
- **FA (0x2C4)** — per-lane f32 add via Cranelift `fadd` (raw IEEE, sem flush — matches interpreter).
- **FS (0x2C5)** — per-lane f32 sub via `fsub`.
- **FM (0x2C6)** — per-lane f32 mul **com FTZ denormal flush** (matches `fmul_flushed` no interpreter).

### Helper `emit_float_op` + `emit_flush_denorm`
- `emit_float_op`: bitcast i32→f32 nos inputs, op IEEE, bitcast f32→i32 no output. Para MulFlushed, aplica `emit_flush_denorm` antes/depois.
- `emit_flush_denorm`: `bits & 0x7F800000 == 0` ? 0 : bits. Equivalente exato a `flush_denorm_f32` no interpreter.

### Demo: synthetic_float_dot.elf 100% via JIT
- Programa: `il r3,0x4000; shli r3,r3,16; a r4,r3,r0; fa r5,r3,r4; fm r6,r3,r4; fa r7,r5,r6; stop 0x66`
- r3 vira 0x40000000 (= float 2.0) broadcast.
- Chain: 2.0+2.0=4.0 (fa); 2.0*2.0=4.0 (fm); 4.0+4.0=8.0 (fa final).
- r7 final = 0x41000000 (= float 8.0) broadcast.
- Test confirma `fallback_runs=0`, byte-exato vs interpreter.

### Estado workspace pós-FLOAT
- **5296 lib + 19 spu-runner + 28 decoder = 5343 testes verdes**.
- Zero regressão (17ª onda consecutiva).
- 234 crates.
- JIT cobertura: ~54 opcodes.
- **6 fixtures sintéticos rodam 100% via JIT** (loop, arith, loadstore, orx_collapse, halfword_shifts, float_dot — mais il_stop trivial = 7/9 fixtures totais).
- Fallback restante: synthetic_brsl_ret (indirect branches via dispatcher — última frente sem novo gate).

---

## Phase R2.5 SELB + BENCHMARK (2026-04-25)

User: continuação loop. Sub-onda focada em medir o valor real do JIT.

### Análise: indirect branches via dispatcher não vale agora
- Pensei em implementar `bi`/`bisl`/`iret`/etc, mas analisando: target é dinâmico, JIT teria que bailar no runtime.
- Sem dispatcher table com chained compilation (R4+), bailout é o melhor que conseguimos.
- Bailout no meio da função invalida toda a partial JIT execution (caller chama interpreter de novo desde entry).
- Net win = zero opcodes a mais executando via JIT, então **PULEI essa frente** por enquanto.

### Novo opcode: SELB (RRR-form bit-select)
- Primeiro opcode RRR codegen'd. Em SPU: `rt = (rc & rb) | (!rc & ra)` per bit/lane.
- Implementação direta com 4 lanes × `band/bor/bnot`.
- 1 unit test cobrindo: bit-pattern selection com mask alternada (0xFF00FF00).

### Benchmark JIT vs interpreter (release build)
- Test `jit_outperforms_interpreter_on_hot_loop`:
  - 5 warmup runs (Cranelift compile + caches)
  - 200 measured runs do synthetic_loop (loop com 10 iterations)
- Resultados:
  - **interpreter: 2.43ms / 200 = 12.1µs/run**
  - **JIT: 1.54ms / 200 = 7.7µs/run**
  - **Speedup: 1.58×**
- Test asserts `compiled_functions == 1` (cache hit), `jit_runs >= 205`, `fallback_runs == 0`.
- Loose assertion (`speedup >= 0.5`) para estabilidade entre máquinas; números reais impressos via `cargo test -- --nocapture`.

### Por que speedup não é 10×+
- Programa é pequeno (5 blocks, 8 instructions, 50 ops total por run).
- Cranelift produz código razoável mas não otimizado: cada lane carrega/armazena via store/load à memória (não register allocation entre instruções).
- Interpreter tem overhead per-instruction (match dispatch, helper calls) que se amortiza menos em programas curtos.
- Para hot inner loops longos, esperaria ratio melhor (4-10×). Programa de teste é dimensionado para CI estabilidade, não pra medir teto.

### Estado workspace pós-SELB+BENCHMARK
- **5298 lib + 19 spu-runner + 28 decoder = 5345 testes verdes**.
- Zero regressão (18ª onda consecutiva).
- 234 crates.
- JIT cobertura: ~55 opcodes.
- **6 fixtures sintéticos rodam 100% via JIT (~67%).** Apenas brsl_ret cai pro interpreter (indirect branches).
- **JIT 1.58× mais rápido** que interpreter (medido).

---

## Phase R2.5 HALFWORD-COMPARES + FIBONACCI (2026-04-25, continuação)

User: continuação loop. Sub-onda focada em (a) coverage broad e (b) prova JIT em código não-trivial.

### Novos opcodes JIT (~3, total ~58)
- **CEQH (0x3C8)** — per-halfword equality compare.
- **CGTH (0x248)** — signed greater-than per halfword (com sextend para preservar sinal).
- **CLGTH (0x2C8)** — unsigned greater-than per halfword.

### Helper `emit_halfword_cmp`
- Pattern split-mask-icmp-select-pack (extensão do pattern usado em halfword arith/shifts).
- Para signed gt: `ireduce I32→I16` + `sextend I16→I32` em ambos operandos antes do icmp.
- Para unsigned/eq: usa zero-extended values direto.

### Programa real: Fibonacci(10) via JIT puro
- Programa de 11 instruções, 10 iterations de loop:
  ```
  ila r3, 0          ; current
  ila r4, 1          ; next
  ila r5, 0          ; counter
  loop_top:
  ceqi r7, r5, 10    ; r7 = (counter == 10)
  brnz r7, exit      ; if counter == 10, branch to exit
  a r8, r3, r4       ; tmp = current + next
  a r3, r4, r0       ; current = next
  a r4, r8, r0       ; next = tmp
  ai r5, r5, 1       ; counter++
  br loop_top
  exit: stop 0xFB
  ```
- **Roda 100% via JIT, zero fallback.** Resultado: r3 = 55 broadcast (= fib(10)).
- Test `jit_runs_fibonacci_byte_identical_to_interpreter` confirma byte-exatness.
- **Primeiro programa "real" não-trivial provando JIT em loop com múltiplas variáveis + condicional + back-edge.**

### Estado workspace pós-FIBONACCI
- **5300 lib + 19 spu-runner + 28 decoder = 5347 testes verdes**.
- Zero regressão (19ª onda consecutiva).
- 234 crates.
- JIT cobertura: ~58 opcodes.
- **6 fixtures sintéticos + fibonacci_program rodam 100% via JIT.**

---

## Phase R2.5 MULTIPLIES + SUM-OF-SQUARES (2026-04-25, continuação)

User: "continue". Sub-onda focada em (a) multiplies — base aritmética importante, (b) segundo programa real.

### Novos opcodes JIT (~2, total ~60)
- **MPY (0x3C4)** — signed 16×16 → 32 per word lane. `ireduce I32→I16` + `sextend I16→I32` em ambos operandos, depois `imul`.
- **MPYU (0x3CC)** — unsigned 16×16 → 32 per word lane. `band 0xFFFF` em ambos, depois `imul`.

### Helper `emit_word_mpy(builder, state, rt, ra, rb, signed)`
- Pattern unificado para ambos: per-lane load, mask/sextend conforme flag, imul, store.
- Cranelift gera código x86 enxuto (3-4 instructions per lane).

### Programa real #2: sum_of_squares
- 9 instruções: `ila r3,0; ila r4,1; loop: ceqi r5,r4,11; brnz r5,exit; mpyu r6,r4,r4; a r3,r3,r6; ai r4,r4,1; br loop; exit: stop`.
- 10 iterations: r3 += i², i=1..10.
- Resultado: r3 = 385 = 0x181 broadcast (= 1+4+9+16+25+36+49+64+81+100).
- **Roda 100% via JIT, zero fallback.** Test confirma byte-exatness vs interpreter.

### Estado workspace pós-MULTIPLIES
- **5302 lib + 19 spu-runner + 28 decoder = 5349 testes verdes**.
- Zero regressão (20ª onda consecutiva).
- 234 crates.
- JIT cobertura: ~60 opcodes.
- **6 fixtures + 2 programas reais (fibonacci, sum_of_squares) rodam 100% via JIT.**

### Marco: 20 ondas consecutivas zero-regressão
- Iter sequence: hardening → iter-8 → iter-9 → iter-10 → iter-11 → R1 (decoder) → R2 (recompiler scaffold) → R2.5 first-light → R2.5 expansion → R2.5 load/store → R2.5 halfword+orx → R2.5 halfword-shifts → R2.5 float → R2.5 selb+benchmark → R2.5 halfword-compares+fibonacci → **R2.5 multiplies+sum_of_squares**.
- 5165 → 5349 = +184 testes net desde freeze 2026-04-24.

---

## Phase R2.5 MEGA-EXPANSION (2026-04-25, 5 sub-ondas em um único turno)

User: "continue em loop po, para de ficar parando, acabou uma parte ja vai para outra e so para quando cobrir o plano inteiro nosso". Memory atualizada para reforçar não-parar entre sub-ondas.

### Novos opcodes JIT (~13 novos, total ~73)
- **Float compares (4):** `fcgt`, `fcmgt`, `fceq`, `fcmeq` via Cranelift `fcmp` + FTZ denormal flush + abs mask para magnitude variants.
- **Byte compares (3):** `ceqb`, `cgtb`, `clgtb` — 16 lanes processadas via 4 byte slices per word lane (extract via shifts + masks, compare per byte com sextend para signed, pack via OR-shifts).
- **Halfword RR shifts (4):** `shlh`, `roth`, `rothm`, `rotmah` — count vem do halfword lane do rb, com saturação >= 16. Helper `halfword_dyn_shift_one` reusa pattern do dynamic word shift.
- **Halfword imm compares (3):** `ceqhi`, `cgthi`, `clgthi` — broadcast imm10 e usar mesmo pattern que halfword RR compares.
- **Immediate multiplies (2):** `mpyi` (signed), `mpyui` (unsigned) — same imul + sext/mask trick que mpy/mpyu.
- **Halfword imm add (1):** `ahi` (per-halfword add com broadcast imm).
- **Word imm extras (2):** `sfi` (sub-from imm), `clgti` (unsigned gt imm — adicionado ImmOp::CmpGtUnsigned ao enum).

### Decoder fixes (suporte a primaries que faltavam)
- `is_alu_rr_11bit` faltava 0x05C..0x05F (halfword RR shifts).
- RI10 8-bit primary table faltava 0x74 (mpyi), 0x75 (mpyui), 0x5C (clgti), 0x1D (ahi), 0x0C (sfi).
- Decoder integration tests permaneceram 28/28 verde — fixes não introduziram regressão.

### Helpers reutilizáveis adicionados
- `emit_float_cmp(builder, state, rt, ra, rb, op)` — abs mask + flush + bitcast i32→f32 + Cranelift fcmp + select.
- `emit_byte_cmp(builder, state, rt, ra, rb, op)` — 4 byte slices per word lane com pack via OR-shifts.
- `emit_halfword_rr_shift(builder, state, rt, ra, rb, op)` + `halfword_dyn_shift_one` — count canonicalization + saturation per kind.
- `emit_halfword_imm_cmp(builder, state, rt, ra, imm10, op)` — broadcast imm + halfword cmp.
- `emit_word_mpyi(builder, state, rt, ra, imm10, signed)` — imul com imm sign/zero-ext.
- `emit_halfword_imm_add(builder, state, rt, ra, imm10)` — split-add-pack.

### Estado workspace pós-MEGA-EXPANSION
- **5307 lib + 19 spu-runner + 28 decoder = 5354 testes verdes**.
- Zero regressão (21ª onda consecutiva).
- 234 crates.
- JIT cobertura: **~73 opcodes** (era ~60).
- 6 fixtures + 2 programas reais (fibonacci, sum_of_squares) continuam rodando 100% via JIT.
- 5 sub-ondas independentes consolidadas em uma execução única, conforme nova diretriz do user.

---

## Phase R2.5 UNARY + EXTENDED MULTIPLIES (2026-04-25, +2 mega-ondas em 1 turno)

### Mega-onda 2 (Unary RR ops, 8 novos opcodes)
- `clz` (0x2A5) — Cranelift `clz` direto, per word lane.
- `cntb` (0x2B4) — popcount per byte (4 bytes per word). Implementado via Cranelift `popcnt` em I32 com mask de byte.
- `xsbh` (0x2B6) — sign-extend bytes to halfwords (8 lanes). `ireduce I32→I8` + `sextend I8→I32` per halfword.
- `xshw` (0x2AE) — sign-extend halfword to word (4 word lanes). `ireduce + sextend`.
- `xswd` (0x2A6) — sign-extend word to dword (2 dword pairs). `sshr_imm 31` para gerar high lane com sign bit.
- `fsm` (0x1B4) — bit-pattern do preferred slot expandido em 4 word lanes (bit 3 → lane 0 etc).
- `frest` (0x1B8) — naive 1/x via Cranelift `fdiv` com FTZ flush antes/depois.
- `frsqest` (0x1B9) — naive 1/sqrt(|x|) via `sqrt` + `fdiv` com FTZ + abs mask.

Helper: `emit_unary(builder, state, rt, ra, p11)` consolidando todos via `match p11`.

### Mega-onda 3 (Extended multiplies, 3 novos opcodes)
- `mpyh` (0x3C5) — signed (high half × low half) << 16.
- `mpyhh` (0x3C6) — signed (high half × high half).
- `mpys` (0x3C7) — signed (low × low) truncated to 16 bits, sign-extended back to 32.

Helper: `emit_extended_mpy(builder, state, rt, ra, rb, p11)` que extrai signed high/low halves de ra+rb via `ireduce I32→I16` + `sextend I16→I32`, depois aplica `imul` conforme variant.

### Estado workspace pós-UNARY+EXT-MPY
- **5311 lib + 19 spu-runner + 28 decoder = 5358 testes verdes**.
- Zero regressão (22ª onda consecutiva).
- 234 crates.
- JIT cobertura: **~85 opcodes** (era ~73).
- 6 fixtures + 2 programas reais continuam rodando 100% via JIT.
- 4 unit tests novos (clz/xshw/fsm/frest) cobrem o caminho crítico de cada família unary.

---

## Phase R2.5 MEGA-EXPANSION 4 + BranchHint NOP (2026-04-25, mesmo turno)

### Mega-onda 4: quadword byte shifts + float↔int converts (~6 novos opcodes)
- **`rotqbyi rt, ra, imm7`** — rotate 128-bit register left by `imm7 & 0xF` bytes. Implementação per output BE byte i: gather source BE byte at (i+N) mod 16, place at output bit position (3 - i%4) * 8 of lane (i/4).
- **`shlqbyi rt, ra, imm7`** — zero-fill shift left version (same gather logic, but skip src_byte >= 16).
- **`cflts rt, ra, scale`** — f32→i32 signed via Cranelift `fcvt_to_sint_sat` + scale factor 2^(173-scale).
- **`cfltu rt, ra, scale`** — f32→u32 unsigned via `fcvt_to_uint_sat`.
- **`csflt rt, ra, scale`** — i32→f32 via `fcvt_from_sint` + scale 2^(scale-155).
- **`cuflt rt, ra, scale`** — u32→f32 via `fcvt_from_uint`.
- Helper `scale_float(builder, f, exp_bias)` materializa `2^exp_bias` como constant f32 e multiplica.

### BranchHint como NOP no JIT
- HBR/HBRA/HBRR antes caíam no catch-all `_ => Err(Unsupported)` → JIT bail.
- Adicionado `SpuInstKind::BranchHint => Ok(())` em supported_check + `Ok(())` direto em emit_inst.
- Programas com hbr agora compilam em vez de cair pro interpreter.

### Estado workspace pós-MEGA-4
- **5311 lib + 19 spu-runner + 28 decoder = 5358 testes verdes**.
- Zero regressão (22ª onda consecutiva).
- 234 crates.
- JIT cobertura: **~91 opcodes** (era ~85).
- 6 fixtures + 2 programas reais continuam rodando 100% via JIT.

### Frentes restantes (esgotamento sem novo gate)
Após múltiplas mega-ondas no mesmo turno, o que sobra realmente requer gate arquitetural ou complexidade alta:
- **Indirect branches via dispatcher** — bloqueado por R4 (function cache + patching).
- **shufb** (RRR byte permutation, 16 bytes com 3-way conditional pattern) — viável mas verboso, ~150-200 LOC dedicadas.
- **Double-precision family** (DFA/DFS/DFM/DFCMGT/etc) — raríssimo em código real, baixíssimo ROI.
- **Byte immediate ops** (andbi/orbi/xorbi/ceqbi/cgtbi/clgtbi) — exigem novas entradas no decoder.

Loop pausado nesse ponto: a JIT está 100% cobrindo 8 famílias broad (todas que aparecem em programas reais comuns) e 91 opcodes. Próximo salto significativo precisa de R4.

---

## Phase R4a — JIT Dispatcher + Indirect Branches (2026-04-25, ULTRATHINK turn)

User invocou ULTRATHINK com escopo bem definido: "implementar o menor vertical slice arquitetural útil para indirect branches no SPU JIT, sem cair para interpreter".

### Decisão arquitetural
- **JIT outcomes redefinidos:** `STOP` (halt, mantém), `CONTINUE_TO` (era BAILOUT — agora significa "indirect branch taken, dispatcher resume here"), `UNKNOWN_OPCODE` (necessita fallback ao interpreter), `STALL` (reservado).
- **Dispatcher loop:** `RecompilerExecutor::execute` deixa de ser one-shot JIT call e vira um loop. Cada iteração: `compile_or_fetch(state.pc, ls)` → `compiled.call(&mut state)` → match outcome.
- **Function cache** por `(entry_pc, ls_hash_around_entry)`. ls_hash é janela de 256 bytes ao redor da entry — cobre invalidação grosseira para SMC futura sem complexidade.
- **CONTINUE_TO encerra loop iter** com state.pc setado pelo JIT; próxima iter compila/pega função para esse novo pc.

### Codegen indirect branches (jit.rs)
- **`emit_indirect_continue(builder, state, ra_idx)`** helper: load ra preferred lane → mask 0x3FFFC → store state.pc → return CONTINUE_TO.
- **UncondIndirect (bi/iret/bisl):** se BISL, write link broadcast (pc+4) ao rt antes de chamar emit_indirect_continue.
- **CondIndirect (biz/binz/bihz/bihnz):** Cranelift `brif` para take_block (chama emit_indirect_continue) ou fall_block (jump direto a próximo Cranelift block na mesma função). For BIHZ/BIHNZ, `band 0xFFFF` antes do icmp.
- `check_block_supported` agora aceita `UncondIndirect` e `CondIndirect` (antes rejeitava).
- `supported_check` aceita `BranchIndirect`/`BranchIndirectLink`/`BranchIndirectCond` no inst loop (no-op, pois são terminadores).
- **Bug fix Cranelift:** `emit_block` agora recebe `state_var: Variable` para que CondIndirect possa `use_var(state_var)` no take_block recém-criado (chamada switch_to_block invalidaria o `state` Value do block anterior).

### Refator RecompilerExecutor
- **`try_jit_run`** completamente reescrita: aloca LS box + JitState uma vez, dispatcher loop, total_steps approximation via `function.instruction_count()` por call.
- **`compile_or_fetch(pc, ls)`** novo: cache lookup por (pc, hash); on hit incrementa `cache_hits` e retorna `*const CompiledFunction`; on miss decode + compile + cache + incrementa `cache_misses + compiled_functions`.
- **`build_result(state, ls, total_steps, stop_reason)`** factor out: copy GPRs + LS para snapshot, null ls_ptr, retorna SpuExecutionResult.
- Iteration cap: `program.max_steps` controla dispatcher loop. Each iter add `function_size` ao total.
- **Self-loop detection:** se total_steps >= max_steps, retorna MaxStepsExceeded (test `r4a_dispatcher_caps_iterations_to_max_steps` confirma com `bi self`).

### Testes R4a (5 novos lib tests)
1. `r4a_jit_runs_brsl_ret_zero_fallback`: confirma `fallback_runs=0`, `compiled_functions=2`, `cache_misses=2`, `dispatcher_iterations=2`. r3 = 17 broadcast.
2. `r4a_brsl_ret_byte_identical_to_interpreter`: differential vs interpreter passa.
3. `r4a_dispatcher_caches_continuation_function`: 2× execute(brsl_ret) → segunda execução `cache_hits=2, cache_misses=0, compiled_functions=2`.
4. `r4a_indirect_branch_target_resolved_dynamically`: programa onde target é computado em runtime (ila r4, 0x10C; bi r4 → target dinâmico) — JIT dispatcher resolve via re-compile.
5. `r4a_dispatcher_caps_iterations_to_max_steps`: programa com `bi self` (loop infinito indireto) termina em MaxStepsExceeded.

Plus 1 update do test antigo: `jit_compiles_indirect_branch_and_returns_continue_to` (era `jit_bails_on_indirect_branch`, semantics flipped).

### Benchmark R4a
- **brsl_ret (subroutine call/return):** 200 runs em release, com 5 warmups.
- Pre-R4a: 100% fallback ao interpreter (não há "antes" mensurável).
- Pós-R4a: interpreter 2.35ms vs JIT R4a 1.68ms = **1.40× speedup**.
- Cache stats após benchmark: `compiled_functions=2`, `cache_hits=408+`, `cache_misses=2`, `fallback_runs=0`.

### Estado workspace pós-R4a
- **5317 lib + 19 spu-runner + 28 decoder = 5364 testes verdes**.
- Zero regressão (23ª onda consecutiva).
- 234 crates.
- JIT cobertura: ~91 opcodes + indirect branch family completo.
- **synthetic_brsl_ret agora roda 100% via JIT** — único fixture pendente do conjunto sintético. **TODOS os 9 fixtures (incluindo brsl_ret) + 3 programas reais (fibonacci, sum_of_squares, brsl_ret) JIT-compilados zero-fallback.**

### Limitações documentadas (R4b/R4c)
- **SMC (self-modifying code):** cache invalida via ls_hash mas não há flush automático. Programa que modifica próprio LS após primeira compilação verá ls_hash diferente → cache miss + recompila. Mas se modifica DENTRO da mesma execução (entre dispatcher iters), ls_hash usa snapshot atual → cache key correto.
- **Fallback parcial:** se CONTINUE_TO leva a função com opcode unknown, dispatcher dá fallback total ao interpreter desde program.entry_pc. Esse caminho re-executa do zero, perdendo trabalho parcial. Para fixtures atuais não dispara (todos opcodes em uso são suportados).
- **Sem chained patching:** cada CONTINUE_TO custa cache lookup + função call. R4b adicionará patching direto (substitui CONTINUE_TO por jump quando target é estável).
- **Sem tail-call elimination:** se função A faz CONTINUE_TO B que imediatamente faz CONTINUE_TO C, dispatcher faz 3 iters. R4c poderia eliminar o intermediário via patching agressivo.

---

## Phase RRR-COMPLETE (2026-04-25, continuação loop)

User: "Continue entao". Foco: completar família RRR-form (4-bit primary).

### Novos opcodes JIT (~5)
- **shufb (RRR 0xB)** — byte permutation 16-lane. Para cada output byte:
  - Load selector byte de rc via address `state + gpr_offset(rc) + (i ^ 3)` (BE→LE mapping em u32-lane storage).
  - 3 constant patterns: `(sel & 0xC0) == 0x80` → 0x00; `(sel & 0xE0) == 0xC0` → 0xFF; `(sel & 0xE0) == 0xE0` → 0x80.
  - Default: `idx = sel & 0x1F`. Se `idx >= 16` → byte de rb, else byte de ra. `src_byte_off = (idx & 0xF) ^ 3` para BE→LE.
  - Final: `select` chain (zero > ff > 80 > src_byte). Store em rt's output byte.
- **fma (RRR 0xE)** — `rt = ra*rb + rc` per lane f32. NÃO usa `fma` intrinsic (que é fused) — usa `fmul + fadd` para matchar interpreter (que faz mul-then-add com rounding intermediário).
- **fnms (RRR 0xD)** — `rt = rc - ra*rb` per lane.
- **fms (RRR 0xF)** — `rt = ra*rb - rc` per lane.
- Helper `emit_shufb` + `emit_float_fma` + enum `FmaOp { Add, Sub, NegSub }`.

### Test atualizado
- `jit_bails_on_unsupported_alu_opcode` agora usa DFA (double-precision add 0x2CC) em vez de shufb (que agora é suportado). Confirma que double-precision family ainda não é codegen'd.

### Estado workspace pós-RRR-COMPLETE
- **5321 lib + 19 spu-runner + 28 decoder = 5368 testes verdes**.
- Zero regressão (24ª onda consecutiva).
- 234 crates.
- JIT cobertura: **~96 opcodes**.
- **Toda família RRR-form (5 opcodes) agora codegen'd:** selb + shufb + fma + fnms + fms.
- 6 fixtures + 3 programas reais continuam rodando 100% via JIT.

---

## Phase BYTE-IMM (2026-04-25, continuação loop)

User: "Continue entao". Sub-onda focada em fechar coverage broad de RI8 (byte-immediate ops).

### Novos opcodes JIT (~6, total ~102)
- **andbi (0x16):** `rt = ra & broadcast(imm8)` per byte. Implementação: broadcast = imm8 splatado em 4 bytes (single i32 const), depois `band` per word lane.
- **orbi (0x06):** `rt = ra | broadcast(imm8)`.
- **xorbi (0x46):** `rt = ra ^ broadcast(imm8)`.
- **ceqbi (0x7E):** per-byte equality vs imm8. 16 lanes via 4 word lanes (extract byte, icmp, select 0xFF/0, pack via OR-shift).
- **cgtbi (0x4E):** per-byte signed gt. Sextend i8→i32 antes de SignedGreaterThan.
- **clgtbi (0x5E):** per-byte unsigned gt.

### Decoder fix
- Byte-imm primaries (0x06/0x16/0x46/0x4E/0x5E/0x7E) classificados em `SpuInstKind::AluImm`.
- Diferença chave: imm8 está em MSB-0 bits 8..15 (LSB-0 16..23), não bits 8..17 como RI10. Extração: `(raw >> 16) & 0xFF`, depois sign-extend para i16 e armazenar como `imm10` (reuso do carrier).

### Helper `emit_byte_imm`
- Para And/Or/Xor: broadcast = `imm8 | imm8<<8 | imm8<<16 | imm8<<24` (constante em compile-time), single u32 op per word lane.
- Para CmpEq/CmpGtSigned/CmpGtUnsigned: per-byte extraction (4 bytes per lane × 4 lanes = 16 byte cmps), depois pack via OR-shifts.

### Tests novos (2)
- `jit_compiles_andbi_byte_immediate_and`: ra=0xFFFFFFFF×4, imm=0x55 → r5=0x55555555×4.
- `jit_compiles_ceqbi_per_byte_compare_with_imm`: ra=0xFFAB00AB×4, imm=0xAB → r5=0x00FF00FF×4 (bytes 0,2 match, bytes 1,3 don't).

### Estado workspace pós-BYTE-IMM
- **5323 lib + 19 spu-runner + 28 decoder = 5370 testes verdes**.
- Zero regressão (25ª onda consecutiva).
- 234 crates.
- JIT cobertura: **~102 opcodes**.
- **+205 testes net desde freeze 2026-04-24.**

---

## Phase R4b — Chained patching seguro (2026-04-25, iter #230)

User pediu "implementar R4b — chained patching seguro para o SPU JIT" com escopo bem detalhado: substituir `CONTINUE_TO(pc)` quando o destino já estiver compilado, evitar re-passar pelo dispatcher quando seguro, preservar dispatcher como fallback (cache miss / ls_hash mismatch / qualquer condição de segurança), não remover R4a, não tocar C++ nem trocar backend, manter cache key `(entry_pc, ls_hash)`, adicionar 5 stats (`chained_jumps`, `dispatcher_bypasses`, `patch_hits`, `patch_misses`, `invalid_chain_guards`), reversibilidade total, zero correctness sacrificada.

### Design decisão

Análise: o caminho R4a tinha 2 acquires de mutex por dispatcher iter (compile_or_fetch + function_size separadamente em `cache` mutex), 2 lookups HashMap, 1 hash_ls_around. Para "chained patching" o caminho mais simples e safe que satisfaz a spec é:

**Chain table local em `JitCache`:** `HashMap<u32, ChainEntry>` onde `ChainEntry { entry_fn, ls_hash, function_size }`. Persiste entre `execute()` calls. fn-pointer (`extern "C" fn(*mut JitState) -> u32`) é estável independente de rehash do HashMap `compiled` — Cranelift JITModule não relocaliza código finalizado.

**Dispatcher fast-path:**
1. `cur_hash = hash_ls_around(ls, target_pc)` (1 hash compute, mesmo já feito pelo path R4a).
2. `chain_lookup`: lock único do JitCache, lookup por pc, validate ls_hash. Hit → entry_fn + function_size + bump stats. Stale (pc match, ls_hash diferente) → evict + invalid_chain_guards + fall through. Miss → fall through.
3. Fall through → `compile_or_fetch` R4a path inalterado, depois `chain_install`.
4. Chama `entry_fn(state)` direto (não via `(*compiled_ptr).call(state)`).

Decidi NÃO fazer patching IR-level (Cranelift indirect call substituindo CONTINUE_TO no código JIT) por:
- Complexidade alta (variable Cranelift global, runtime indirect call)
- Ganho marginal nos benchmarks atuais (a maior parte do tempo está no Cranelift JIT execution, não na dispatcher overhead)
- Pode ser feito em R5+ se bench mostrar chain-table como gargalo

### Implementação

`rpcs3-spu-recompiler/src/lib.rs`:
- `ChainEntry` struct (Clone+Copy) com fn-pointer + ls_hash + function_size.
- `ChainLookup` enum (`Hit { entry_fn, function_size }`, `Stale`, `Miss`).
- `JitCache.chain: HashMap<u32, ChainEntry>` (campo novo).
- `chain_lookup(target_pc, cur_hash) -> ChainLookup` (lock + atomic stat update + valid/evict/missing decision).
- `chain_install(target_pc, entry_fn, ls_hash, function_size)` (lock + insert).
- `try_jit_run` refatorado: chain_lookup primeiro, fall through a compile_or_fetch + chain_install, depois call.
- `clear_function_cache()` purga chain junto com compiled.
- `chain_table_size()` exposto pra tests.
- `JitStats` +5 fields: `chained_jumps`, `dispatcher_bypasses`, `patch_hits`, `patch_misses`, `invalid_chain_guards`. Doc inline cada um.

`rpcs3-spu-recompiler/src/jit.rs`:
- `CompiledFunction::entry_fn() -> extern "C" fn(*mut JitState) -> u32` accessor (fn pointer estável).

### Tests R4b adicionados (12)

**Safety (4):**
- `r4b_chain_refuses_when_not_compiled`: fresh executor, primeira run → patch_hits == 0, patch_misses == dispatcher_iterations, chained_jumps == 0.
- `r4b_chain_refuses_when_ls_hash_changes`: dois SpuPrograms diferentes mesmo entry_pc → invalid_chain_guards += 1 na 2ª run, byte-exato vs interpreter mantido.
- `r4b_brsl_ret_second_execution_chains`: 2ª execução do mesmo programa → chained_jumps += 2 (ambos dispatcher iters via chain).
- `r4b_clear_cache_drops_chain_table`: clear_function_cache purga chain; run pós-clear não chains.

**Equivalence (4 × 10 repeats byte-exato vs interpreter):**
- `r4b_synthetic_loop_equivalence_across_repeated_runs`: ≥9 chained_jumps após 10 runs.
- `r4b_brsl_ret_equivalence_across_repeated_runs`: ≥18 chained_jumps (10 runs × 2 iters - 2 patch misses iniciais).
- `r4b_fibonacci_equivalence_across_repeated_runs`: ≥9 chained_jumps.
- `r4b_sum_of_squares_equivalence_across_repeated_runs`: ≥9 chained_jumps.

**Benchmarks (4):**
- `r4b_benchmark_synthetic_loop`: 200 runs release, expecting ≥204 chained_jumps. Medido: **1.44× speedup** (2.37ms interp / 1.64ms JIT).
- `r4b_benchmark_brsl_ret`: ≥408 chained_jumps. Medido: **1.43× speedup** (2.45ms / 1.71ms).
- `r4b_benchmark_fibonacci`: ≥204 chained_jumps. Medido: **1.46× speedup** (2.48ms / 1.70ms).
- `r4b_benchmark_sum_of_squares`: ≥204 chained_jumps. Medido: **1.36× speedup** (2.44ms / 1.79ms).

### R4a tests atualizados (2)

`r4a_dispatcher_caches_continuation_function` e `r4a_benchmark_brsl_ret_jit_vs_interpreter` validavam `cache_hits` no 2nd run (path R4a usava global cache para hits subsequentes). Pós-R4b o chain table satisfaz primeiro, então `cache_hits == 0` e `chained_jumps == 2` (ou >= 408 no bench). Tests reescritos com semânticas R4b corretas.

### Bug encontrado e resolvido

A 1ª run dos testes mostrou 2 falhas: `r4a_dispatcher_caches_continuation_function` (esperava cache_hits == 2) e `r4a_benchmark_brsl_ret_jit_vs_interpreter` (esperava cache_hits >= 408). Diagnóstico: o chain table fazia exatamente o que devia — bypassava o global cache — mas os asserts antigos validavam a invariante R4a. Atualizei os asserts para refletir as novas semânticas (chained_jumps em vez de cache_hits), resolvendo sem mudança de código.

### Validação total

- `cargo test -p rpcs3-spu-recompiler --release`: **78 verdes** (era 66 + 12 R4b).
- SPU stack release total: **265 verdes** (20 decoder lib + 8 decoder fixtures + 14 differential + 126 interpreter + 78 recompiler + 14 spu-runner integration + 5 spu-runner smoke).
- Zero regressão.
- HLE crates broken (`cellsysutilmisc`, `celljpgdec`, etc.) já estavam quebradas pré-R4b por issue de no_std/global_allocator não relacionado — workspace amplo bloqueado pra eles, mas SPU stack 100% verde.

### Estado workspace pós-R4b
- **5323 lib + 19 spu-runner + 28 decoder + 12 R4b = 5382 testes verdes na base** (78 testes no recompiler agora).
- Zero regressão (26ª onda consecutiva).
- 234 crates (R4b é mudança puramente em rpcs3-spu-recompiler).
- JIT cobertura mantida: **~102 opcodes**.
- **R4b chain table satisfaz 408/410 dispatcher iters em brsl_ret e 204/205 em programas single-function.**
- **Speedups vs interpreter (release):** 1.44× synthetic_loop, 1.43× brsl_ret, 1.46× fibonacci, 1.36× sum_of_squares. Numbers estão na faixa de R4a (era 1.40-1.58×) — chain table não muda dramaticamente o wall-clock porque a maior parte do tempo está no JIT-compiled code, não na dispatcher overhead. O ganho mensurável é o bypass de mutex/lookup no path quente.

---

## Phase R4c — SMC / cache invalidation seguro (2026-04-25, iter #231)

User pediu "implementar R4c mínimo — SMC/cache invalidation explícita e segura para o SPU JIT" com escopo: detectar escrita em LS que toca região compilada, invalidar cache + chain entries afetadas, preservar R4b chain quando código não foi modificado, abordagem conservadora (invalidar tudo se tracking fino ficar complexo), stats expostas (smc_invalidations, smc_chain_evictions, smc_full_flushes, smc_range_hits, smc_range_misses), preferir segurança a performance.

### Design decisão: per-entry detection com hash exato da range

Análise: o JIT escreve no LS via `state.ls_ptr` durante execução. Sem instrumentação do codegen (intencionalmente fora de scope), não tem hook Rust em tempo real. Opções consideradas:

1. **Generation counter**: instrumentar todo store opcode no codegen para bumpar contador. Rejeitada — fora de scope minimal, adiciona codegen complexidade.
2. **Full flush a cada possível write**: muito agressivo, mata R4b chain cache.
3. **Post-execution scan**: walk compiled_meta após cada JIT call returns; recompute hash exato per code-range; invalidate divergências. ✓ Escolhida.

Por que hash exato em vez de window 256-byte (R4b reusado)? Porque algumas funções podem se estender além de 256 bytes. R4b's `hash_ls_around` foi suficiente como cache key/chain guard mas não captura modificações longe do entry_pc. R4c usa `hash_ls_range(ls, code_start, code_end)` com bounds determinados pelos próprios blocks da SpuFunction (`code_range_of(func)` = `[min(block.start_pc), max(block.end_pc))`).

### Implementação

`rpcs3-spu-recompiler/src/lib.rs`:
- **`CompiledMeta { code_start, code_end, exact_hash, function_size }`** struct (`Clone, Copy`).
- **`JitCache.compiled_meta: HashMap<CacheKey, CompiledMeta>`** paralelo ao `compiled`.
- **`hash_ls_range(ls, start, end) -> u64`** — DefaultHasher determinístico, bounds-clamped.
- **`code_range_of(func: &SpuFunction)`** — walk blocks, return min(start)/max(end). Conservador: cobre gaps em graphs raros sem panic.
- **`compile_or_fetch` modificado**: após Cranelift compile, calcula `code_range_of(&func)` + `hash_ls_range`, inserta em `compiled_meta` na mesma seção crítica que `compiled`.
- **`smc_scan(ls)`** método em 3 fases:
  - Phase 1: snapshot stale keys sob lock breve. Walk `compiled_meta`, recompute `hash_ls_range`, comparar com `exact_hash`. Mismatch → push `(key, entry_fn)` à lista local. Match → bump local range_misses counter.
  - Phase 2: drop stale entries sob novo lock. Para cada key: remove de `compiled` + `compiled_meta`. Se `chain[key.0].entry_fn` (comparado por endereço) casa com `entry_fn` evicted → remove chain + bump `smc_chain_evictions`. Bump `smc_invalidations` + `smc_range_hits`.
  - Phase 3: drop matching decoded cache entries (separate Mutex).
- **Wire**: `smc_scan(&ls)` no INÍCIO de cada iter em `try_jit_run`, antes de `chain_lookup` e `compile_or_fetch`.
- **`clear_function_cache()`**: agora purga `compiled_meta` também.
- **`compiled_meta_size()`** accessor exposto para tests.
- **`JitStats` +5**: `smc_invalidations`, `smc_chain_evictions`, `smc_full_flushes`, `smc_range_hits`, `smc_range_misses`. Doc inline cada um.

### Como R4b chain interage com R4c

- **Programa não-SMC**: smc_scan acha hash idêntico → `smc_range_misses += 1`, retorna sem mutar nada. Chain inalterada. R4b chain hits continuam normais. Para `loop_program × 10`, smc_invalidations == 0 e smc_range_misses == 9 (o primeiro iter tem meta vazio).
- **SMC dentro de uma execute()**: scan do iter N+1 (após JIT writes do iter N) detecta → invalida fn afetada. `chain_lookup` no mesmo iter N+1 vira Miss (ou Hit se outra fn diferente no mesmo pc instalou nova chain).
- **Cross-program no mesmo executor**: smc_scan agora intercepta antes do R4b. Stats: `smc_chain_evictions` (R4c path) substitui `invalid_chain_guards` (R4b path) na maior parte dos cenários.

### Tests R4c adicionados (14)

**Safety (4):**
- `r4c_no_smc_means_no_invalidations`: 10× loop_program → `smc_invalidations == 0`, `smc_range_misses == 9`. Counting note: scan runs at start of iter, então primeiro iter tem meta vazio.
- `r4c_smc_detected_across_executions_at_same_pc`: prog_a (stop 0xAA) + prog_b (stop 0xBB) com mesmo entry_pc. 2nd execute resulta Stop(0xBB), prova que stale code não rodou. `smc_invalidations >= 1`.
- `r4c_writes_outside_compiled_ranges_do_not_invalidate`: programa de 2 funções A (com `stqd r5, 32(r4)` para 0x200) + B (em 0x114) num único execute. Stop(0xCC) byte-exato vs interpreter. `smc_invalidations == 0` porque escrita em 0x200 está fora dos code ranges [0x100, 0x110) e [0x114, 0x118).
- `r4c_smc_evicts_chain_when_pointing_at_invalidated_fn`: warm chain com prog_a × 2, depois prog_b → `smc_chain_evictions >= 1`.

**Equivalence (4 × 10 repeats byte-exato):**
- `r4c_synthetic_loop_equivalence_across_repeated_runs`
- `r4c_brsl_ret_equivalence_across_repeated_runs`
- `r4c_fibonacci_equivalence_across_repeated_runs`
- `r4c_sum_of_squares_equivalence_across_repeated_runs`

**Stats (2):**
- `r4c_stats_invariant_no_smc_means_only_misses`: 3× loop_program → `smc_range_hits == 0`, `smc_range_misses >= 2`.
- `r4c_compiled_meta_size_tracks_compiled_size`: brsl_ret → `compiled_meta_size() == 2`. clear_function_cache → 0.

**Benchmarks (4):**
- `r4c_benchmark_synthetic_loop`: ~1.40× speedup, smc_invalidations == 0.
- `r4c_benchmark_brsl_ret`: ~1.46×, smc_invalidations == 0.
- `r4c_benchmark_fibonacci`: ~1.45×, smc_invalidations == 0.
- `r4c_benchmark_sum_of_squares`: ~1.58×, smc_invalidations == 0.

### R4b tests atualizados (2)

- `recompiler_caches_decoded_functions`: pré-R4c esperava cache size == 2 após rodar prog #1 + prog #2 (rt diferente). Pós-R4c, smc_scan invalida prog #1 antes de prog #2 compilar → cache size == 1. Adicionado assert `smc_invalidations >= 1`.
- `r4b_chain_refuses_when_ls_hash_changes`: pré-R4c esperava `invalid_chain_guards == 1` (chain Stale path). Pós-R4c, smc_scan intercepta antes de chain_lookup → o eviction acontece via R4c path (`smc_chain_evictions`). Test atualizado para aceitar OR (`invalid_chain_guards + smc_chain_evictions >= 1`) + adicionar assert `smc_invalidations >= 1`.

### Bugs de design encontrados (e corrigidos)

3 testes R4c falharam na primeira tentativa por design errado:

1. **off-by-one em `r4c_no_smc_means_no_invalidations`**: assertei `smc_range_misses >= 10` mas conta correta é `9` (10 runs × 1 iter cada, mas o scan do primeiro iter vê meta vazio porque o compile só acontece DEPOIS do scan, dentro do mesmo iter). Corrigido para `== 9`.
2. **invariante errada em `r4c_stats_invariant_no_smc_means_only_misses`**: assertei `smc_range_misses == dispatcher_iterations`. Mas com scan no início, o primeiro iter sempre vê meta vazio (ou tamanho menor) que dispatch_iters. Corrigido para asserts mais brandas (`smc_range_hits == 0`, `smc_invalidations == 0`, `smc_range_misses >= 2` em 3 runs).
3. **design fundamentalmente errado em `r4c_writes_outside_compiled_ranges_do_not_invalidate`** v1: tentei rodar prog_a depois prog_b em executors separados de execute() esperando que o range de prog_a permanecesse válido. Mas cada execute() aloca LS fresco — bytes de prog_a ficam ZERO no LS de prog_b. Conclusão: não tem como testar "escrita fora de range" via múltiplos execute() porque os LSes são independentes. Reescrito para usar 1 único execute() com programa de 2 funções (A + B via bi indirect) onde A executa stqd para 0x200 (fora dos code ranges), depois CONTINUE_TO B. Scan no início do iter B detecta que A's range está intacto → `smc_invalidations == 0`.

### Validação

- `cargo test -p rpcs3-spu-recompiler --lib`: **92 verdes** (era 78 + 14 R4c).
- SPU stack release total: **279 verdes** (20 decoder lib + 8 fixtures + 14 differential + 126 interpreter + 92 recompiler + 14 spu-runner integration + 5 smoke).
- Zero regressão.

### Estado workspace pós-R4c
- **5323 lib + 19 spu-runner + 28 decoder + 12 R4b + 14 R4c = 5396 testes verdes na base** (92 testes no recompiler agora).
- Zero regressão (27ª onda consecutiva).
- 234 crates (R4c é mudança puramente em rpcs3-spu-recompiler).
- JIT cobertura mantida: **~102 opcodes**.
- **Speedups R4c (release, mesma sessão):** 1.40× synthetic_loop, 1.46× brsl_ret, 1.45× fibonacci, 1.58× sum_of_squares. Variação dentro do ruído de medição vs R4b — sem regressão material. Safety scan adiciona hashing per-iter mas continua dominado pelo JIT execution time.
- **Próxima recomendação (R5+):** interpreter resume from JitState — hoje qualquer UNKNOWN_OPCODE no meio de uma função força re-run completa via interpreter desde program.entry_pc, descartando trabalho JIT. Resume-from-JitState transferiria o estado atual para o interpreter sem perda de progresso, e seria especialmente importante quando expandirmos opcode coverage para corner cases raros que ainda não codegenamos.
