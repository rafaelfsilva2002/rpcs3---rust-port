...
//! oracle (18th oracle). Byte-identical to PUTL (R8.4e) except
//! cmd=0x26 (PUTL | MFC_FENCE_MASK).
...
const SPU_SENTINEL: u32 = 0xC0FF_EEBF;
const FIXTURE_NAME: &str = "single_spu_dma_putlf_v1";
...
#[test]
fn r8_4f_b_single_spu_dma_putlf_v1_replay_validated_byte_identical() {
    ...
    let events = parse_jsonl_trace(&raw).expect("R8.4f-b: parser must accept PUTLF trace");
    ...
    let cmd = mfc_cmds[0];
    assert_eq!(cmd.cmd, 0x26, "PUTLF cmd code (= PUTL | MFC_FENCE_MASK)");
    ...
    let plan = apply_mfc_dma_pre_replay(&events, &trace_path, &dma_dir, program)
        .expect("apply_mfc_dma_pre_replay must succeed for PUTLF");
    ...
    eprintln!(
        "[R8.4f-b SUCCESS] {FIXTURE_NAME} replay-validated (18th oracle):\n  \
         PUTLF cmd=0x26 (= PUTL data path + fence modifier stripped)\n  \
