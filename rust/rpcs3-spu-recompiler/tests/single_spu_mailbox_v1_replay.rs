//! R5.9e.7 — first replay-validated fixture integration test.
//!
//! Loads `behavior-freeze/fixtures/spu/traces/single_spu_mailbox_v1.jsonl`
...
//!
//! This test is the load-bearing acceptance gate that flips
//! `REPLAY_VALIDATED_TRACE_EXISTS` from False to True. If it passes
//! end-to-end, the project HAS its first replay-validated SPU trace.
//!
//! Pre-capture: this test is `#[ignore]`d because the fixture file is
//! authored at capture-time, not check-in time. Once R5.9e.7 capture
//! lands, the `#[ignore]` is removed AND `check_trace_fixtures.py`
//! flips its `REPLAY_VALIDATED_TRACE_EXISTS` constant.
...
/// Resolves the path to `behavior-freeze/fixtures/spu/traces/single_spu_mailbox_v1.jsonl`
/// relative to the workspace root (this crate is `rust/rpcs3-spu-recompiler/`).
fn fixture_trace_path() -> PathBuf {
    ...
    p.push("spu");
    p.push("traces");
    p.push("single_spu_mailbox_v1.jsonl");
    p
}

/// Centralized image dir per § F.4 plan: `behavior-freeze/fixtures/spu/images/`.
...
#[test]
fn r5_9e_7_single_spu_mailbox_v1_replay_validated_byte_identical() {
    ...
    assert!(
        ...
        "fixture trace not found at {}\n\
         (R5.9e.7 capture must produce this file before this test passes)",
        trace_path.display(),
    );
    assert!(
        images_dir.exists(),
        "fixture images dir not found at {}\n\
         (R5.9e.7 capture must produce per-image .spuimg side-files in this dir)",
        images_dir.display(),
    );
    ...
    // ===== 3. Verify acceptance criteria 4-6 of R5.9e.7 spec. =====
    // Criterion 4: zero spu_wrch ch21 (MFC_Cmd) events.
    ...
    assert!(
        ...
        "fixture must NOT contain any spu_wrch ch21 (MFC_Cmd) events; \
         found {} — fixture is DMA-bound and cannot be replay-validated",
        mfc_cmd_events.len(),
    );

    // Criterion 5: at least 1 spu_wrch ch28 (OUT_MBOX) event. The
    // simplified v3 fixture is single-round (PPU pushes 0x100, SPU
    // computes 0x129, writes OUT_MBOX, halts) so exactly 1 ch28 is
    // expected. Earlier 2-round design needed PPU OUT_MBOX drain
    // which clashed with PSL1GHT's cooperative-thread MMIO mapping.
    ...
    assert!(
        out_mbox_count >= 1,
        "fixture must have ≥1 spu_wrch ch28 (OUT_MBOX) event for the mailbox handshake; \
         got {out_mbox_count}",
    );
    ...
    // ===== 5. Locate the spu_image event(s). Must be exactly 1. =====
    ...
    // total_steps is internal-accounting, not part of the
    // byte-identical state contract. The canonical comparison
    // surface (`diff_snapshots`) excludes it: PC, GPRs, LS,
    // channels, and park_state determine equivalence; how many
    // micro-ops each backend reports is a per-backend tally
    // (the JIT counts dispatcher iterations + JIT prefix steps,
    // the interpreter counts raw instructions). Both should be
    // > 0 — that's the only meaningful invariant here.
    assert!(
    ...
    eprintln!(
        "[R5.9e.7 SUCCESS] single_spu_mailbox_v1 replay-validated:\n  \
         target_spu={}\n  \
