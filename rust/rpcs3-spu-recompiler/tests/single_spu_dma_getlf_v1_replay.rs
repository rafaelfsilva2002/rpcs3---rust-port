...
//! oracle (16th oracle). Byte-identical to GETL (R8.4c) except
//! cmd=0x46 (GETL | MFC_FENCE_MASK).
//!
//! Per RPCS3 `do_list_transfer`, the fence bit is stripped
//! before the per-element copy (`args.cmd & ~0xf`), so the data
//! path is byte-identical to plain GETL. Fence ordering effects
//! (waiting for prior commands on the same tag) don't surface
...
const CANONICAL_STATUS: u32 = 0xDF1E_EA7F;
const FIXTURE_NAME: &str = "single_spu_dma_getlf_v1";
...
#[test]
fn r8_4f_a_single_spu_dma_getlf_v1_replay_validated_byte_identical() {
    ...
    let events = parse_jsonl_trace(&raw).expect("R8.4f-a: parser must accept GETLF trace");
    ...
    let cmd = mfc_cmds[0];
    assert_eq!(cmd.cmd, 0x46, "GETLF cmd code (= GETL | MFC_FENCE_MASK)");
    ...
    let plan = apply_mfc_dma_pre_replay(&events, &trace_path, &dma_dir, program)
        .expect("apply_mfc_dma_pre_replay must succeed for GETLF");
    ...
    assert_eq!(&ls[lo1..hi1], chunk_0.as_slice(), "GETLF element 0 in LS at cumulative offset");
    assert_eq!(&ls[lo2..hi2], chunk_1.as_slice(), "GETLF element 1 in LS at cumulative offset");
    ...
    eprintln!(
        "[R8.4f-a SUCCESS] {FIXTURE_NAME} replay-validated (16th oracle):\n  \
         GETLF cmd=0x46 tag={} size={} (= GETL data path + fence modifier stripped at do_list_transfer)\n  \
