...
Third PPU-only entry in the behavior-freeze fixture set (after
`single_mutex_v1` and `single_sema_v1`). Exercises the LV2
`sys_event_queue_*` + `sys_event_port_*` syscall path end-to-end
through `rpcs3-emu-core::run_self`, validating the R10.6
`EventRegistry` trait impl and its dispatcher arms
(#128, #129, #130, #134, #135, #136, #137, #138) against a real
...
toolchain itself is NOT redistributed — only the source code and
resulting binary that exercises its API.

## Provenance

- Author: this project's contributors (via the autonomous R10.1.f
  iteration).
