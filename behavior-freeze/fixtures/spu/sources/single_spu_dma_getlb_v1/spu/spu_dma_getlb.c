// single_spu_dma_getlb_v1 — SPU side
// CC0 1.0 (public domain). See ../LICENSE.md.
//
// R8.4f-a — MFC GETLB list-DMA + barrier dispatch (cmd=0x45).
// Identical to R8.4c GETL SPU side except:
// - `MFC_GETLB` instead of `MFC_GETL` (cmd 0x45 vs 0x44).
// - `STATUS_MASK` 0xC0DEFABB (last byte BB = Barrier mnemonic)
//   instead of GETL's 0xC0DEFADA, so a regression that
//   accidentally accepts cmd=0x45 but reuses GETL's mask would
//   surface as a wrong canonical status.
...
#define MFC_GETLB      ((uint32_t)0x45)
#define STATUS_MASK    ((uint32_t)0xC0DEFABBu)
