// single_spu_dma_getlb_v1 — PPU side
// CC0 1.0 (public domain). See LICENSE.md.
//
// R8.4f-a — first MFC GETLB list-DMA + barrier oracle (15th
// oracle target). Identical to R8.4c GETL except cmd=0x45 in
// the SPU dispatcher (GETL | MFC_BARRIER_MASK). Per
// `do_list_transfer` the per-element byte copy is identical;
// per `do_dma_check` the barrier persistence has no
// observable effect for a single-SPU fresh-tag fixture.
...
extern const u8  spu_dma_getlb_bin[];
extern const u32 spu_dma_getlb_bin_size;
...
int main(s32 argc, const char *argv[])
{
    ...
    if (ret) {
        printf("[dma_getlb_v1] sysSpuInitialize: 0x%08x\n", ret);
        return 1;
    }

    sysSpuImage spu_image;
    ret = sysSpuImageImport(&spu_image, spu_dma_getlb_bin, 0);
    if (ret) {
        printf("[dma_getlb_v1] sysSpuImageImport: 0x%08x\n", ret);
        return 1;
    }

    sysSpuThreadGroupAttribute group_attr;
    sysSpuThreadGroupAttributeInitialize(group_attr);
    sysSpuThreadGroupAttributeName(group_attr, "dma_getlb_v1");
    ...
    if (ret) {
        printf("[dma_getlb_v1] sysSpuThreadGroupCreate: 0x%08x\n", ret);
        return 1;
    }
    ...
    if (ret) {
        printf("[dma_getlb_v1] sysSpuThreadInitialize: 0x%08x\n", ret);
        return 1;
    }

    ret = sysSpuThreadGroupStart(group_id);
    if (ret) {
        printf("[dma_getlb_v1] sysSpuThreadGroupStart: 0x%08x\n", ret);
        return 1;
    }
    ...
    if (ret) {
        printf("[dma_getlb_v1] sysSpuThreadGroupJoin: 0x%08x\n", ret);
        return 1;
    }

    // Predicted: cause=0x1, status=0xDF1EEA3B
    printf("[dma_getlb_v1] OK cause=0x%x status=0x%x\n",
