// single_spu_dma_putlf_v1 — PPU side
// CC0 1.0 (public domain). See LICENSE.md.
//
// R8.4f-b — first MFC PUTLF list-DMA + fence oracle.
...
extern const u8  spu_dma_putlf_bin[];
extern const u32 spu_dma_putlf_bin_size;
...
int main(s32 argc, const char *argv[])
{
    ...
    if (ret) { printf("[dma_putlf_v1] sysSpuInitialize: 0x%08x\n", ret); return 1; }

    sysSpuImage spu_image;
    ret = sysSpuImageImport(&spu_image, spu_dma_putlf_bin, 0);
    if (ret) { printf("[dma_putlf_v1] sysSpuImageImport: 0x%08x\n", ret); return 1; }

    sysSpuThreadGroupAttribute group_attr;
    sysSpuThreadGroupAttributeInitialize(group_attr);
    sysSpuThreadGroupAttributeName(group_attr, "dma_putlf_v1");
    ...
    if (ret) { printf("[dma_putlf_v1] sysSpuThreadGroupCreate: 0x%08x\n", ret); return 1; }
    ...
    if (ret) { printf("[dma_putlf_v1] sysSpuThreadInitialize: 0x%08x\n", ret); return 1; }

    ret = sysSpuThreadGroupStart(group_id);
    if (ret) { printf("[dma_putlf_v1] sysSpuThreadGroupStart: 0x%08x\n", ret); return 1; }

    u32 cause, spu_sentinel;
    ret = sysSpuThreadGroupJoin(group_id, &cause, &spu_sentinel);
    if (ret) { printf("[dma_putlf_v1] sysSpuThreadGroupJoin: 0x%08x\n", ret); return 1; }
    ...
    u32 ea_status = combined ^ 0xBEEFCAFFu;

    // Predicted: cause=0x1, spu=0xc0ffeebf, ea_status=0xa12fda7f
    printf("[dma_putlf_v1] OK cause=0x%x spu=0x%x ea_status=0x%x\n",
