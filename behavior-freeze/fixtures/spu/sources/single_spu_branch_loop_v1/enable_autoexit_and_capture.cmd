@echo off
REM ============================================================
REM single_spu_branch_loop_v1 — RPCS3 trace capture (auto-exit)
REM ============================================================
REM
REM Identical workflow to single_spu_mailbox_v1's capture script
REM (R5.9e.7). See sibling fixture for full inline documentation.
REM
...
if exist "%TEMP%\single_spu_branch_loop_v1.jsonl" del /q "%TEMP%\single_spu_branch_loop_v1.jsonl"
if exist "%TEMP%\single_spu_branch_loop_v1.images" rmdir /s /q "%TEMP%\single_spu_branch_loop_v1.images"
if exist "%TEMP%\single_spu_branch_loop_v1.jsonl.images" rmdir /s /q "%TEMP%\single_spu_branch_loop_v1.jsonl.images"
echo [Step 2] Cleaned prior trace artifacts
echo.

REM === Step 3: set env var ===
set RPCS3_SPU_TRACE_JSONL=%TEMP%\single_spu_branch_loop_v1.jsonl
echo [Step 3] Capturing trace to: %RPCS3_SPU_TRACE_JSONL%
echo          .self: %~dp0build\single_spu_branch_loop_v1.self
echo.

REM === Step 4: launch RPCS3 (will auto-exit when program finishes) ===
echo [Step 4] Launching RPCS3...
"R:\bin\rpcs3.exe" --headless "%~dp0build\single_spu_branch_loop_v1.self"
...
if exist "%TEMP%\single_spu_branch_loop_v1.jsonl.images" (
    echo [OK] Images dir: %TEMP%\single_spu_branch_loop_v1.jsonl.images
    dir /b "%TEMP%\single_spu_branch_loop_v1.jsonl.images"
) else if exist "%TEMP%\single_spu_branch_loop_v1.images" (
    echo [OK] Images dir: %TEMP%\single_spu_branch_loop_v1.images
    dir /b "%TEMP%\single_spu_branch_loop_v1.images"
) else (
