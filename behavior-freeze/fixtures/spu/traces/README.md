...
**Status atual:** **6 REPLAY-VALIDATED FIXTURES LANDED.** R5.9e.7 introduced `single_spu_mailbox_v1` (the first oracle); R5.11 + R5.11b (post-R5 closure additive expansions) added three more; R6.4b-replay added the fifth (`single_spu_mailbox_multi_v1`); R6.6 added the sixth (`game_like_mailbox_signal_v1`) — all six `diff_snapshots(interp, jit).is_identical()` byte-identically:
