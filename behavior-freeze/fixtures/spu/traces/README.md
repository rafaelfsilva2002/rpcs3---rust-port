...
**Status atual:** **9 REPLAY-VALIDATED FIXTURES LANDED.** R5.9e.7 introduced `single_spu_mailbox_v1` (the first oracle); R5.11 + R5.11b (post-R5 closure additive expansions) added three more; R6.4b-replay added the fifth (`single_spu_mailbox_multi_v1`); R6.6 added the sixth (`game_like_mailbox_signal_v1`); R6.7 A.5 added the seventh (`single_spu_dma_get_v1` — first DMA-bound oracle); R8.1 added the eighth (`single_spu_dma_put_v1` — first PUT-bound oracle, symmetric to GET but inverts DMA direction); R8.2 added the ninth (`single_spu_dma_get_multi_v1` — first multi-DMA oracle, two queued GETs + ALL wait mode) — all nine `diff_snapshots(interp, jit).is_identical()` byte-identically:
